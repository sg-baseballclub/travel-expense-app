...
/* === subject-settlement-budget enhancement v20260910b === */
if (typeof window.on !== 'function') {
  window.on = function(el, eventName, handler) {
    if (el && typeof el.addEventListener === 'function') el.addEventListener(eventName, handler);
  };
}

state.subjectOrderDirty = !!state.subjectOrderDirty;
state.budgetMeta = state.budgetMeta || {
  reportDate: '',
  directorName: '',
  accountantName: ''
};

function getSubjectFiscalYear_() {
  return Number((els.subjectFiscalYear && els.subjectFiscalYear.value) || state.currentFiscalYear || getCurrentFiscalYear_());
}

function getBudgetFiscalYear_() {
  return Number((els.budgetFiscalYear && els.budgetFiscalYear.value) || state.currentFiscalYear || getCurrentFiscalYear_());
}

function getBudgetDefaultMeta_() {
  return {
    reportDate: '',
    directorName: '',
    accountantName: ''
...
function ensureEnhancementStyleBlock_() {
  if (document.getElementById('accountingEnhancementStyleV20260910b')) return;
  const style = document.createElement('style');
  style.id = 'accountingEnhancementStyleV20260910b';
  style.textContent = `
    .subject-enhanced-hidden { display: none !important; }
    .subject-board { display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 16px; }
    .subject-panel { border: 1px solid #dbe5f1; border-radius: 16px; background: #f8fbff; padding: 14px; }
    .subject-panel.expense { background: #fff8f6; }
    .subject-panel-head { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 10px; }
    .subject-panel-title { font-weight: 700; font-size: 15px; }
    .subject-panel-note { color: #64748b; font-size: 12px; }
    .subject-dropzone { display: grid; gap: 8px; min-height: 96px; }
    .subject-card { display: grid; grid-template-columns: 28px 1fr auto; align-items: center; gap: 10px; border: 1px solid #d7e3f4; border-radius: 12px; padding: 10px 12px; background: #ffffff; cursor: grab; box-shadow: 0 1px 2px rgba(15, 23, 42, 0.04); }
    .subject-card.selected { border-color: #2563eb; box-shadow: 0 0 0 2px rgba(37, 99, 235, 0.15); }
    .subject-card[draggable="true"]:active { cursor: grabbing; }
    .subject-card.disabled-subject { opacity: 0.62; }
    .subject-drag-handle { color: #94a3b8; font-size: 18px; text-align: center; user-select: none; }
    .subject-card-main { min-width: 0; }
    .subject-card-line { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
    .subject-card-code { font-weight: 700; color: #0f172a; }
    .subject-card-name { color: #334155; }
    .subject-card-note { color: #64748b; font-size: 12px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .subject-order-badge { min-width: 44px; text-align: center; border-radius: 999px; background: #dbeafe; color: #1d4ed8; font-weight: 700; padding: 4px 8px; font-size: 12px; }
    .subject-type-toolbar { display: flex; flex-wrap: wrap; gap: 8px; align-items: center; }
    .subject-toolbar-spacer { flex: 1 1 auto; }
    .budget-meta-card, .settlement-approval-card { margin-top: 16px; }
  `;
  document.head.appendChild(style);
}

function ensureSubjectEnhancedLayout_() {
  ensureEnhancementStyleBlock_();
  const page = document.getElementById('page-subjects');
  if (!page) return;
  const actions = page.querySelector('.page-actions');
  if (actions && !document.getElementById('saveSubjectOrderButton')) {
    const reloadButton = document.getElementById('reloadSubjectsButton');
    const html = '<button id="saveSubjectOrderButton" class="button secondary" type="button" disabled>並び順を保存</button><button id="resetSubjectOrderButton" class="button secondary" type="button" disabled>元に戻す</button>';
    if (reloadButton) {
      reloadButton.insertAdjacentHTML('afterend', html);
    } else {
      actions.insertAdjacentHTML('beforeend', html);
    }
  }
  const filterLabel = document.querySelector('label[for="subjectTypeFilter"]');
  const filterContainer = document.getElementById('subjectTypeFilter') ? document.getElementById('subjectTypeFilter').closest('.field') : null;
  if (filterContainer) {
    filterContainer.classList.add('subject-type-toolbar');
  }
  const tableCard = page.querySelector('.table-card');
  const tableWrap = tableCard ? tableCard.querySelector('.table-wrap') : null;
  if (tableWrap) {
    tableWrap.classList.add('subject-enhanced-hidden');
    if (!document.getElementById('subjectGroupedBoard')) {
      tableWrap.insertAdjacentHTML('afterend', '<div id="subjectGroupedBoard" class="subject-board"></div>');
    }
  }
  const sortLabel = Array.from(document.querySelectorAll('#subjectFormModal label span')).find(function(node) {
    return String(node.textContent || '').trim() === '表示順';
  });
  if (sortLabel) sortLabel.textContent = '表示順（手入力も可）';
}

function ensureBudgetEnhancedLayout_() {
  ensureEnhancementStyleBlock_();
  const page = document.getElementById('page-budget');
  if (!page) return;
  const actions = page.querySelector('.page-actions');
  if (actions && !document.getElementById('saveBudgetMetaButton')) {
    const exportButton = document.getElementById('exportBudgetPdfButton');
    const html = '<button id="saveBudgetMetaButton" class="button secondary" type="button">記載情報を保存</button>';
    if (exportButton) {
      exportButton.insertAdjacentHTML('beforebegin', html);
    } else {
      actions.insertAdjacentHTML('beforeend', html);
    }
  }
  if (!document.getElementById('budgetMetaCard')) {
    const summaryGrid = page.querySelector('.summary-grid');
    const html = `
      <div id="budgetMetaCard" class="card form-card budget-meta-card">
        <div class="card-title-row">
          <div>
            <div class="card-title">予算書記載情報</div>
            <div class="inline-note">年度ごとに保存され、PDF出力時に反映されます。</div>
          </div>
        </div>
        <div class="form-grid">
          <label class="field compact-field">
            <span>予算報告日</span>
            <input type="date" id="budgetReportDate" />
          </label>
          <label class="field compact-field">
            <span>監督</span>
            <select id="budgetDirectorName"></select>
          </label>
          <label class="field compact-field">
            <span>会計</span>
            <select id="budgetAccountantName"></select>
          </label>
        </div>
      </div>
    `;
...
const _cacheEnhancedEls_v20260910b_ = cacheEnhancedEls_;
cacheEnhancedEls_ = function() {
  if (typeof _cacheEnhancedEls_v20260910b_ === 'function') _cacheEnhancedEls_v20260910b_();
  Object.assign(els, {
    saveSubjectOrderButton: document.getElementById('saveSubjectOrderButton'),
    resetSubjectOrderButton: document.getElementById('resetSubjectOrderButton'),
    subjectGroupedBoard: document.getElementById('subjectGroupedBoard'),
    saveBudgetMetaButton: document.getElementById('saveBudgetMetaButton'),
    budgetReportDate: document.getElementById('budgetReportDate'),
    budgetDirectorName: document.getElementById('budgetDirectorName'),
    budgetAccountantName: document.getElementById('budgetAccountantName')
  });
};

const _ensureEnhancedLayout_v20260910b_ = ensureEnhancedLayout_;
ensureEnhancedLayout_ = function() {
  if (typeof _ensureEnhancedLayout_v20260910b_ === 'function') _ensureEnhancedLayout_v20260910b_();
  ensureSubjectEnhancedLayout_();
  ensureBudgetEnhancedLayout_();
  cacheEnhancedEls_();
};

function normalizeSubjectRowOrders_() {
  const typeOrder = ['収入', '支出'];
  const grouped = {};
  typeOrder.forEach(function(type) { grouped[type] = []; });
  (state.subjectMasterRows || []).forEach(function(row) {
    const type = row['収支区分'] || '';
    if (!grouped[type]) grouped[type] = [];
    grouped[type].push(row);
  });
  Object.keys(grouped).forEach(function(type) {
    grouped[type].forEach(function(row, index) {
      row['表示順'] = index + 1;
    });
  });
  const merged = [];
  typeOrder.forEach(function(type) {
    (grouped[type] || []).forEach(function(row) { merged.push(row); });
  });
  Object.keys(grouped).forEach(function(type) {
    if (typeOrder.indexOf(type) !== -1) return;
    (grouped[type] || []).forEach(function(row) { merged.push(row); });
  });
  state.subjectMasterRows = merged;
}

function setSubjectOrderDirty_(dirty) {
  state.subjectOrderDirty = !!dirty;
  if (els.saveSubjectOrderButton) els.saveSubjectOrderButton.disabled = !dirty;
  if (els.resetSubjectOrderButton) els.resetSubjectOrderButton.disabled = !dirty;
}

function moveSubjectRowWithinType_(type, fromCode, toCode) {
  const sourceRows = (state.subjectMasterRows || []).filter(function(row) {
    return (row['収支区分'] || '') === type;
  });
  const fromIndex = sourceRows.findIndex(function(row) { return row['科目コード'] === fromCode; });
  const toIndex = sourceRows.findIndex(function(row) { return row['科目コード'] === toCode; });
  if (fromIndex < 0 || toIndex < 0 || fromIndex === toIndex) return;
  const moved = sourceRows.splice(fromIndex, 1)[0];
  sourceRows.splice(toIndex, 0, moved);
  const untouched = (state.subjectMasterRows || []).filter(function(row) {
    return (row['収支区分'] || '') !== type;
  });
  state.subjectMasterRows = untouched.concat(sourceRows).sort(function(a, b) {
    const orderMap = { '収入': 0, '支出': 1 };
    const aType = Object.prototype.hasOwnProperty.call(orderMap, a['収支区分']) ? orderMap[a['収支区分']] : 9;
    const bType = Object.prototype.hasOwnProperty.call(orderMap, b['収支区分']) ? orderMap[b['収支区分']] : 9;
    if (aType !== bType) return aType - bType;
    return 0;
  });
  normalizeSubjectRowOrders_();
  setSubjectOrderDirty_(true);
  if (state.selectedSubject) {
    const selected = state.subjectMasterRows.find(function(row) {
      return row['科目コード'] === state.selectedSubject['科目コード'];
    });
    if (selected) state.selectedSubject = selected;
  }
  renderSubjectTable();
  if (state.selectedSubject) renderSubjectDetail(state.selectedSubject);
}

function renderSubjectCardsForType_(type, rows, selectedCode) {
  const panelClass = type === '収入' ? 'income' : 'expense';
  const cardHtml = rows.length ? rows.map(function(row) {
    const code = row['科目コード'] || '';
    const selectedClass = code === selectedCode ? ' selected' : '';
    const disabledClass = String(row['使用可否']) === 'false' ? ' disabled-subject' : '';
    return `
      <div class="subject-card${selectedClass}${disabledClass}" draggable="true" data-subject-type="${escapeAttribute(type)}" data-subject-code="${escapeAttribute(code)}">
        <div class="subject-drag-handle">☰</div>
        <div class="subject-card-main">
          <div class="subject-card-line">
            <span class="subject-card-code">${escapeHtml(code)}</span>
            <span class="subject-card-name">${escapeHtml(row['科目名'] || '')}</span>
            ${String(row['使用可否']) !== 'false' ? '<span class="badge yes">使用中</span>' : '<span class="badge none">停止</span>'}
          </div>
          <div class="subject-card-note">${escapeHtml(row['備考'] || '備考なし')}</div>
        </div>
        <div class="subject-order-badge">${escapeHtml(String(row['表示順'] || 0))}</div>
      </div>
    `;
  }).join('') : '<div class="empty-detail">該当する科目がありません。</div>';
  return `
    <section class="subject-panel ${panelClass}">
      <div class="subject-panel-head">
        <div>
          <div class="subject-panel-title">${escapeHtml(type)}科目</div>
          <div class="subject-panel-note">ドラッグ＆ドロップで順番を変更できます。</div>
        </div>
        <div class="inline-note">${rows.length}件</div>
      </div>
      <div class="subject-dropzone" data-subject-dropzone="${escapeAttribute(type)}">${cardHtml}</div>
    </section>
  `;
}

function bindSubjectBoardEvents_() {
  if (!els.subjectGroupedBoard) return;
  let dragCode = '';
  let dragType = '';
  els.subjectGroupedBoard.querySelectorAll('.subject-card').forEach(function(card) {
    card.addEventListener('click', function(event) {
      if (event.target && event.target.closest && event.target.closest('.subject-drag-handle')) return;
      const code = card.dataset.subjectCode || '';
      const row = (state.subjectMasterRows || []).find(function(item) { return item['科目コード'] === code; });
      if (row) renderSubjectDetail(row);
    });
    card.addEventListener('dragstart', function() {
      dragCode = card.dataset.subjectCode || '';
      dragType = card.dataset.subjectType || '';
    ...
    card.addEventListener('drop', function(event) {
      event.preventDefault();
      const targetCode = card.dataset.subjectCode || '';
      const targetType = card.dataset.subjectType || '';
      if (!dragCode || !targetCode || dragCode === targetCode || dragType !== targetType) return;
      moveSubjectRowWithinType_(targetType, dragCode, targetCode);
    });
  });
  els.subjectGroupedBoard.querySelectorAll('[data-subject-dropzone]').forEach(function(zone) {
...
const _renderSubjectTable_v20260910b_ = renderSubjectTable;
renderSubjectTable = function() {
  ensureSubjectEnhancedLayout_();
  cacheEnhancedEls_();
  if (!els.subjectGroupedBoard) {
    return _renderSubjectTable_v20260910b_();
  }
  const rows = state.subjectMasterRows || [];
  if (els.subjectCountLabel) els.subjectCountLabel.textContent = `${rows.length}件`;
  const selectedCode = state.selectedSubject ? state.selectedSubject['科目コード'] : '';
  const filteredType = els.subjectTypeFilter ? els.subjectTypeFilter.value || '' : '';
  const incomeRows = rows.filter(function(row) { return row['収支区分'] === '収入' && (!filteredType || filteredType === '収入'); });
  const expenseRows = rows.filter(function(row) { return row['収支区分'] === '支出' && (!filteredType || filteredType === '支出'); });
  els.subjectGroupedBoard.innerHTML = [
    renderSubjectCardsForType_('収入', incomeRows, selectedCode),
    renderSubjectCardsForType_('支出', expenseRows, selectedCode)
  ].join('');
  bindSubjectBoardEvents_();
};

async function saveSubjectOrder() {
  if (!state.currentUser) return showToast('利用者を選択してください', 'error');
  const fiscalYear = getSubjectFiscalYear_();
  const rows = (state.subjectMasterRows || []).map(function(row) {
    return {
      subjectCode: row['科目コード'] || '',
      type: row['収支区分'] || '',
      sortOrder: Number(row['表示順'] || 0),
  ...
  }).filter(function(row) { return row.subjectCode; });
  showLoading('科目の並び順を保存しています...');
  try {
    await apiPost('accounting/reorderSubjects', {
      currentUser: state.currentUser,
      fiscalYear: fiscalYear,
      rows: rows
    });
    setSubjectOrderDirty_(false);
    showToast('科目の並び順を保存しました', 'success');
    await loadSubjects();
    await loadSubjectMaster();
  } catch (error) {
    showToast(error.message, 'error');
...
async function resetSubjectOrder() {
  setSubjectOrderDirty_(false);
  await loadSubjectMaster();
}

function getBudgetMetaFromForm_() {
  return {
    reportDate: els.budgetReportDate ? els.budgetReportDate.value || '' : '',
    directorName: els.budgetDirectorName ? els.budgetDirectorName.value || '' : '',
    accountantName: els.budgetAccountantName ? els.budgetAccountantName.value || '' : ''
  };
}

function populateBudgetOfficerSelect_(select, selectedValue) {
  if (!select) return;
  const users = (state.users || []).map(function(user) { return (user && user.name ? String(user.name) : '').trim(); }).filter(Boolean);
  const uniqueUsers = Array.from(new Set(users));
  const current = selectedValue || select.value || '';
...
function renderBudgetMetaForm_() {
  const meta = Object.assign(getBudgetDefaultMeta_(), state.budgetMeta || {});
  if (els.budgetReportDate) els.budgetReportDate.value = meta.reportDate || '';
  populateBudgetOfficerSelect_(els.budgetDirectorName, meta.directorName);
  populateBudgetOfficerSelect_(els.budgetAccountantName, meta.accountantName);
}

async function loadBudgetMetaFromServer_(fiscalYear) {
  try {
    const result = await apiGet('accounting/getBudgetMeta', { fiscalYear: fiscalYear });
    state.budgetMeta = Object.assign(getBudgetDefaultMeta_(), result.meta || {});
  } catch (error) {
    state.budgetMeta = getBudgetDefaultMeta_();
    showToast(error.message, 'error');
  }
  renderBudgetMetaForm_();
}

async function saveBudgetMetaToServer_(silent) {
  if (!state.currentUser) return;
  const fiscalYear = getBudgetFiscalYear_();
  state.budgetMeta = getBudgetMetaFromForm_();
  await apiPost('accounting/saveBudgetMeta', {
    currentUser: state.currentUser,
    fiscalYear: fiscalYear,
    meta: state.budgetMeta
  });
  if (!silent) showToast('予算書記載情報を保存しました', 'success');
}

function bindBudgetMetaAutosave_() {
  [els.budgetReportDate, els.budgetDirectorName, els.budgetAccountantName].filter(Boolean).forEach(function(el) {
    if (el.dataset.budgetMetaBound === 'true') return;
    el.dataset.budgetMetaBound = 'true';
    el.addEventListener('change', function() {
      state.budgetMeta = getBudgetMetaFromForm_();
    });
  });
...
const _bindEvents_v20260910b_ = bindEvents;
bindEvents = function() {
  if (typeof _bindEvents_v20260910b_ === 'function') _bindEvents_v20260910b_();
  on(els.saveSubjectOrderButton, 'click', saveSubjectOrder);
  on(els.resetSubjectOrderButton, 'click', resetSubjectOrder);
  on(els.saveBudgetMetaButton, 'click', function() {
    saveBudgetMetaToServer_(false).catch(function(error) {
      showToast(error.message, 'error');
    });
  });
  bindBudgetMetaAutosave_();
};

const _loadUsers_v20260910b_ = loadUsers;
loadUsers = async function() {
  const result = await _loadUsers_v20260910b_();
  renderBudgetMetaForm_();
  renderSettlementMetaForm_();
  bindBudgetMetaAutosave_();
  bindSettlementMetaAutosave_();
...
const _loadBudgetPage_v20260910b_ = loadBudgetPage;
loadBudgetPage = async function(options) {
  ensureBudgetEnhancedLayout_();
  cacheEnhancedEls_();
  renderBudgetMetaForm_();
  bindBudgetMetaAutosave_();
  const result = await _loadBudgetPage_v20260910b_(options);
  ensureBudgetEnhancedLayout_();
  cacheEnhancedEls_();
  await loadBudgetMetaFromServer_(getBudgetFiscalYear_());
  bindBudgetMetaAutosave_();
  return result;
};

const _generateBudgetPdf_v20260910b_ = generateBudgetPdf;
generateBudgetPdf = async function() {
  if (!state.currentUser) return showToast('利用者を選択してください', 'error');
  const fiscalYear = requireValue(els.budgetFiscalYear.value, '年度を選択してください');
  clearBudgetPdfResult();
  state.budgetMeta = getBudgetMetaFromForm_();
...
async function loadSettlementMetaFromServer_(fiscalYear) {
  try {
    const result = await apiGet('accounting/getSettlementMeta', { fiscalYear: fiscalYear });
    const serverMeta = Object.assign(getDefaultSettlementMeta_(), (result && result.meta) || {});
    const serverNotes = (result && result.notes && typeof result.notes === 'object') ? result.notes : {};
    state.settlementMeta = serverMeta;
    state.settlementRows = (state.settlementRows || []).map(function(row) {
      const cloned = Object.assign({}, row);
      const key = [cloned.type || '', cloned.subjectCode || ''].join('|');
      if (Object.prototype.hasOwnProperty.call(serverNotes, key)) cloned.note = serverNotes[key] || '';
      return cloned;
    });
    renderSettlementMetaForm_();
    renderSettlementTable();
...
const _loadSettlementPage_v20260910b_ = loadSettlementPage;
loadSettlementPage = async function() {
  const result = await _loadSettlementPage_v20260910b_();
  await loadSettlementMetaFromServer_(getSettlementFiscalYear_());
  return result;
};

const _saveSettlementInput_v20260910b_ = saveSettlementInput;
saveSettlementInput = async function() {
  const fiscalYear = getSettlementFiscalYear_();
  state.settlementMeta = getSettlementMetaFromForm_();
  try {
    showLoading('決算書入力内容を保存しています...');
    await apiPost('accounting/saveSettlementMeta', {
      currentUser: state.currentUser,
      fiscalYear: fiscalYear,
      meta: state.settlementMeta,
      notes: getSettlementNotesMap_()
    });
    clearUnsavedContext('settlement');
    setUnsavedBaseline('settlement', getSettlementSnapshot());
...
const _generateSettlementPdf_v20260910b_ = generateSettlementPdf;
generateSettlementPdf = async function() {
  if (!state.currentUser) return showToast('利用者を選択してください', 'error');
  const fiscalYear = requireValue((els.settlementFiscalYear && els.settlementFiscalYear.value) || state.currentFiscalYear, '年度を選択してください');
  state.settlementMeta = getSettlementMetaFromForm_();
  clearSettlementPdfResult();
...
const _init_v20260910b_ = init;
init = async function() {
  ensureEnhancedLayout_();
  cacheEnhancedEls_();
  await _init_v20260910b_();
  ensureEnhancedLayout_();
  cacheEnhancedEls_();
  bindBudgetMetaAutosave_();
  setSubjectOrderDirty_(false);
